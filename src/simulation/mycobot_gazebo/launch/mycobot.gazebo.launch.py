...
def generate_launch_description():
    ...
    default_world_file = 'medicine_shelf_demo.world'
    ...
    # Spawn the robot (delayed to let Gazebo fully load before spawning)
    ...
    # Move the arm into a slightly bent home pose after the robot is spawned.
    start_home_pose_cmd = TimerAction(
        period=12.0,
        actions=[ExecuteProcess(
            cmd=[
                ...
                '{joint_names: [link1_to_link2, link2_to_link3, link3_to_link4, link4_to_link5, link5_to_link6, link6_to_link6_flange], points: [{positions: [0.15, -0.45, 0.65, -0.25, 0.20, 0.0], time_from_start: {sec: 3, nanosec: 0}}]}'
            ],
            output='screen'
        )]
    )
    ...
    ld.add_action(start_home_pose_cmd)
